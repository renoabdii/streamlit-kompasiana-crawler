...
# Fungsi MongoDB
def save_to_mongodb(data, db_name="db_bulutangkis", collection_name="test"):
...
def load_articles_from_mongodb(db_name="db_bulutangkis", collection_name="test"):
    client = MongoClient("mongodb://localhost:27017/")
    db = client[db_name]
    collection = db[collection_name]
...
def plot_top_words_line(top_words):
    ...
    ax.set_xlabel("Kata")
    ax.set_ylabel("Frekuensi")
    ax.set_title("10 Kata Paling Sering Muncul (Grafik Garis)")
    plt.xticks(rotation=45)
    plt.grid(True)
    plt.tight_layout()
    return fig
...
if contents:
    ...
    if st.session_state.get("run_mode") == "jadwal":
        fig = plot_top_words_line(top_words)
    else:
        fig = plot_top_words_bar(top_words)
    st.pyplot(fig)
else:
